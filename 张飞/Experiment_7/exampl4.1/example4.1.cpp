...
int main()
{
	cout << "The number of all student:" << Student::total << endl;//无对象时可用类名：：静态成员名形式访问
	Student *p = new Student(13);//用指针申请动态空间并得到对象*p
	cout << "The number of all student:" << Student::total << endl;//用类名：：静态成员名形式访问
	cout << "The number of all student:" << p->total << endl;//用指针所指向的对象访问
	delete p;//通过指针删除动态对象，析构一次
	cout << "The number of all student:" << Student::total << endl;
	Student s[2];//用类名：：静态成员名形式访问,定义两个对象，构造函数调用两次
